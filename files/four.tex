...
This question is hard to answer. In practice, we implemented (1) common binary functions listed in Table~\ref{table:vxi1}; (2) basic vector operations like INSERT VECTOR ELT, EXTRACT VECTOR ELT and BUILD VECTOR\@. All the meaningful test IR files we wrote work properly under this operations set.
...
In-place Lowering allows us to copy the vector between registers or shift the vector within the register boundary. But it is different from vector element promotion. Refer to the Figure~\ref{figure:v8i4_v8i8}, vector element promotion would require shifting different element with different offsets.
